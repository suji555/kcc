...
class kcBERT_custom(nn.Module):
    ...
    def forward(self, src_input_sentence, src_label=None):
        ...
        attention_mask = (src_input_sentence != 0)  # torch.Size([batch_size, max_length]) or torch.Size([batch_size, max_length*2])
        if not src_label==None:
            # Get BERT embedding values
            batch_size = src_input_sentence.shape[0]
            with torch.no_grad():
                if src_label.shape[-1]==2:
                    sen1, sen2 = torch.chunk(src_input_sentence, 2, dim=1)  # torch.Size([batch_size, max_length])
                    embedding_pred_1 = self.bert_embedding(sen1)    # torch.Size([batch_size, max_length, hidden_size])
                    embedding_pred_2 = self.bert_embedding(sen2)
                    label1, label2 = torch.chunk(src_label, 2, dim=-1)  # torch.Size([batch_size, 1])
                    label1 = label1.squeeze()   # torch.Size([batch_size])
                    label2 = label2.squeeze()
                    # one-hot encoding
                    processed_label_1 = torch.zeros(batch_size, self.num_labels).to(self.device)
                    processed_label_2 = torch.zeros(batch_size, self.num_labels).to(self.device)
                    ...
                    processed_label = torch.zeros(batch_size, self.num_labels)
            ...
            if src_label.shape[-1]==2:
                ...
                with torch.no_grad():
                    ...
                    embedding_pred = emb1 + emb2    # DoubleTensor
            # BERT process
            attention_mask = self.bert.get_extended_attention_mask(attention_mask, 
                                                                attention_mask.shape, self.device)
            out = self.bert_encoder(embedding_pred.float(), attention_mask=attention_mask)  # out: tuple(torch.Size([batch_size, max_length, hidden_size]))
            out = self.bert_classifier(self.bert_dropout(self.bert_pooler(out[0])))
